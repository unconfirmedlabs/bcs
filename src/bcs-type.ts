import { fromBase58, fromBase64, fromHex, toBase58, toBase64, toHex } from "./utils.js";
...
export function isSerializedBcs(
	obj: unknown,
): obj is SerializedBcs<unknown> {
	return (
		!!obj && typeof obj === "object" && (obj as any)[SERIALIZED_BCS_BRAND] === true
	);
}
...
export function fixedSizeBcsType<
	T,
	Input = T,
...
export function dynamicSizeBcsType<
	T,
	Input = T,
	const Name extends string = string,
>({
	...
	return new BcsType<string, string, Name>({
		...
		validate: (value) => {
			if (typeof value !== "string") {
				throw new TypeError(
					`Invalid ${options.name} value: ${value}. Expected string`,
				);
			}
			options.validate?.(value);
		},
	...
	constructor({ fields, ...options }: BcsEnumOptions<T, Name>) {
		...
		super({
			read: (reader) => {
				...
				if (!enumEntry) {
					throw new TypeError(
						`Unknown value ${index} for enum ${options.name}`,
					);
				}
...
> {
	constructor({ fields, name, ...options }: BcsTupleOptions<T, Name>) {
		super({
			name:
				name ?? (`(${fields.map((t) => t.name).join(", ")})` as never),
			serializedSize: (values) => {
				let total = 0;
			...
			validate: (value) => {
				...
				if (value.length !== fields.length) {
					throw new TypeError(
						`Expected array of length ${fields.length}, found ${value.length}`,
					);
				}
			},
		});
